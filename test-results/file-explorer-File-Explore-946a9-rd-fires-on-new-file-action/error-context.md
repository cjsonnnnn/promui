...
- generic:
  ...
  - dialog "Create New YAML File" [ref=e11]:
    ...
    - textbox "prometheus" [active] [ref=e16]
    - generic [ref=e17]:
...
```ts
  357 |     // The list should still be present afterwards.
  358 |     await expect(page.getByText(/Config Files/).first()).toBeVisible()
  359 |   })
  360 | 
  361 |   test('FM-29 Config directory path displayed', async ({ page }) => {
  362 |     await expect(page.getByText('Config Directory')).toBeVisible()
  363 |     // The badge below the label should contain the path.
